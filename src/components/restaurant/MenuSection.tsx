...
const MenuSection = ({ menu, photos, reviews, menuUrl, restaurant }: MenuSectionProps) => {
  ...
  const topMatch = session && itemMatchDetails ? 
    Object.entries(itemMatchDetails).reduce((top, [itemId, details]) => {
      return (!top || details.score > top.score) ? { itemId, ...details } : top;
    }, null as null | { itemId: string; score: number; reason?: string }) : null;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {menuToDisplay[0].items.map((item) => (
                  <MenuItem
                    key={item.id}
                    item={item}
                    matchDetails={session ? {
                      ...(itemMatchDetails[item.id] || { score: 50, matchType: 'neutral' }),
                      isTopMatch: topMatch?.itemId === item.id
                    } : null}
                  />
                ))}
              </div>
